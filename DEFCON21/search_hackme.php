...
<body>
<?php
	/* The prototype of a document in the database:
	...
	if (isset($_GET['searchbox']) && !empty($_GET['searchbox'])) {
		...
		$result = "<h4>Result(s) for your search query <em>" . $searchbox . "</em>:</h4>\n";
		try {
			...
			$cursor = $collection->find(array('$where' => $searchbox)); // ahem...
...
<p>Search <input type="text" name="searchbox" id="searchbox" /> <input type="submit" name="submitbutton" value="Submit" /></p>
</form>
<div id="results">
<?php echo $result; ?>
</div>
